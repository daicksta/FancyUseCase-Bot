...
%
	...
	\section{Auslegung}\label{sec:auslegung}
		...
		% Handelsübliche Schrittmotoren besitzen eine Schrittweite bzw. Winkelauflösung von \SI{1,8}{\degree}.
		% Wenn eine 
	In diesem Abschnitt werden die benötigte Kraft der Muskeln sowie die Form des Getriebes berechnet und beschrieben.
	\subsection{Auslegung der künstlichen Muskeln}
	...
	Dazu wurde in eigener Recherche ermittelt, dass die maximalen Geschwindigkeiten (\(v_{Kugel}\)) einer Billiradkugel bei ungefähr 40 km/h, also 11,1 m/s, liegt. 
	Die Gewichte der Kugeln wurde aus der Aufgabenstellung entnommen, dabei wurde die größte Masse angenommen, um alle Arten an Kugeln mindestens mit einer Geschwindigkeit von 11,1 m/s bewegen zu können.
	Diese Masse beträgt \(m_{Kugel}\) = 204 g.
	Die Masse des Arms (\(m_{Arm}\)) wurde mithilfe des CAD-Programms Solidworks ermittelt. Sie liegt bei 1,1 kg.
	...
	\begin{equation}
		v_{Arm} = \frac{\SI{0,204}{\kilogram} \cdot \SI{11,1}{\metre\per\second}}{\SI{1,105}{\kilogram}} = \SI{2,05}{\metre\per\second}
	...
	So ergibt sich eine minimal benötige Kraft der künstlichen Muskeln von 289,72 N. 
